              ...
              <tr key={key}>
                ...
                <td>
                  {p.watchOnly ? (
                    ...
                    <span title="Your list works as it is. Putting its URL in this profile's bio is what proves the list is yours.">
                ...
                <td class="row" style="justify-content:flex-end">
                  ...
                  <Button onClick={() => void send({ type: "run:now", profileKey: key })} title="Read the page now and compare it with the last snapshot, instead of waiting for the timer">
                    Check now
                  </Button>
                  {!p.watchOnly && adapterFor(p.platform).supportsBio && (
                    <Button
                      onClick={async () => {
                        const r = await send<{ ok: boolean; reason?: string }>({ type: "verify:profile", profileKey: key });
                        alert(r.ok ? "Verified." : (r.reason ?? "Not verified"));
                      }}
                      title="Read your bio on the platform and check it links to your published list, which is what proves the list is yours"
                    >
                      Check my bio link
                    </Button>
                  )}
